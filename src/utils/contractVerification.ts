...
export const verifyContract = async (params: VerifyContractParams) => {
	...
	const response = await fetch(
		`${ETHERSCAN_V2_API}?chainid=${chainId}&module=contract&action=verifysourcecode&apikey=${process.env.ETHERSCAN_API_KEY}`,
		{
			method: 'POST',
			body: formData
		}
...
export const getContractSourceCode = async (
	...
	const response = await fetch(url, {
		...
		.catch(console.error)
	return (
		(response as { result: { SourceCode: string }[] }).result[0]?.SourceCode ??
		null
	)
}
