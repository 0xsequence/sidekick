...
export class GoogleKmsSigner extends ethers.AbstractSigner {
    ...
    async signTransaction(transaction: Deferrable<ethers.TransactionRequest>): Promise<string> {
        ...
        const signature = await this.signDigest(
            ethers.keccak256(serialize(resolved as UnsignedTransaction))
        )

        return serialize(resolved as UnsignedTransaction, signature)
    }
