...
export async function stopRewards(fastify: FastifyInstance) {
    fastify.post<{
        ...
        try {
            const { chainId, contractAddress } = request.params as StopRewardsRequestParams;
            ...
            await rewardQueue.removeRepeatable({
                ...
                every: parseInt(rewardData.every) * 60 * 1000
            });
            ...
            return reply.code(500).send({
                error: error instanceof Error ? error.message : 'Failed to stop rewards'
            });
        }
    });
} 