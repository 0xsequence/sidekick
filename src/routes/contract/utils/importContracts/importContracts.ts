...
export async function importContracts(fastify: FastifyInstance) {
	fastify.post<{
		...
		async (request, reply) => {
			if (!process.env.BUILDER_API_SECRET_KEY) {
				return reply.code(500).send({
					result: {
						data: null,
						error:
							'BUILDER_API_SECRET_KEY is not set, in order to interact with your Sequence Project you need to set this value in the config file'
					}
				})
			}
			try {
				...
				const response = await fetch(
					'https://api.sequence.build/rpc/Builder/ListContracts',
					{
						method: 'POST',
						headers: {
