...
type erc721ItemsSetNameAndSymbolRequestBody = {
  tokenName: string, tokenSymbol: string,
  waitForReceipt?: boolean
}

type erc721ItemsSetNameAndSymbolRequestParams = {
  chainId: string
  contractAddress: string
}

type erc721ItemsSetNameAndSymbolResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc721ItemsSetNameAndSymbolSchema = {
  tags: ['erc721Items'],
  body: {
    type: 'object',
...
export async function erc721ItemsSetNameAndSymbol(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc721ItemsSetNameAndSymbolRequestParams
    Body: erc721ItemsSetNameAndSymbolRequestBody
    Reply: erc721ItemsSetNameAndSymbolResponse
  }>(
    '/write/erc721Items/:chainId/:contractAddress/setNameAndSymbol',
    { schema: erc721ItemsSetNameAndSymbolSchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        const { tokenName, tokenSymbol, waitForReceipt } = request.body
        ...
        const encodedData = contract.interface.encodeFunctionData('setNameAndSymbol', [
        ...
        logStep(request, 'Sending setNameAndSymbol transaction...')
        const txResponse: TransactionResponse = await signer.sendTransaction(
          tx,
          { waitForReceipt: waitForReceipt ?? false }
        )
        txHash = txResponse.hash
        logStep(request, 'SetNameAndSymbol transaction sent', { txHash: txResponse.hash })
