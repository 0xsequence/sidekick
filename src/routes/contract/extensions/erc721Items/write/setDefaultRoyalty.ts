...
type erc721ItemsSetDefaultRoyaltyRequestBody = {
  receiver: string, feeNumerator: string,
  waitForReceipt?: boolean
}

type erc721ItemsSetDefaultRoyaltyRequestParams = {
  chainId: string
  contractAddress: string
}

type erc721ItemsSetDefaultRoyaltyResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc721ItemsSetDefaultRoyaltySchema = {
  tags: ['erc721Items'],
  body: {
    type: 'object',
    required: ['receiver', 'feeNumerator'],
    properties: {
...
export async function erc721ItemsSetDefaultRoyalty(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc721ItemsSetDefaultRoyaltyRequestParams
    Body: erc721ItemsSetDefaultRoyaltyRequestBody
    Reply: erc721ItemsSetDefaultRoyaltyResponse
  }>(
    '/write/erc721Items/:chainId/:contractAddress/setDefaultRoyalty',
    { schema: erc721ItemsSetDefaultRoyaltySchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        const { receiver, feeNumerator, waitForReceipt } = request.body
        ...
        const encodedData = contract.interface.encodeFunctionData('setDefaultRoyalty', [
          receiver, BigInt(feeNumerator)
        ])
        ...
        logStep(request, 'Sending setDefaultRoyalty transaction...')
        const txResponse: TransactionResponse = await signer.sendTransaction(
          tx,
          { waitForReceipt: waitForReceipt ?? false }
        )
        txHash = txResponse.hash
        logStep(request, 'SetDefaultRoyalty transaction sent', { txHash: txResponse.hash })
        ...
        await txService.createTransaction({
          ...
          txHash: txHash,
          isDeployTx: false,
          args: [receiver, String(feeNumerator)],
