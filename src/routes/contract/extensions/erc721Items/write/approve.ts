...
type erc721ItemsApproveRequestBody = {
  account: string, id: string,
  waitForReceipt?: boolean
}

type erc721ItemsApproveRequestParams = {
  chainId: string
  contractAddress: string
}

type erc721ItemsApproveResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc721ItemsApproveSchema = {
  tags: ['erc721Items'],
  body: {
    type: 'object',
    required: ['account', 'id'],
...
export async function erc721ItemsApprove(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc721ItemsApproveRequestParams
    Body: erc721ItemsApproveRequestBody
    Reply: erc721ItemsApproveResponse
  }>(
    '/write/erc721Items/:chainId/:contractAddress/approve',
    { schema: erc721ItemsApproveSchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        const { account, id, waitForReceipt } = request.body
