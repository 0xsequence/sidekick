...
type erc721ItemsInitializeRequestBody = {
  owner: string, tokenName: string, tokenSymbol: string, tokenBaseURI: string, tokenContractURI: string, royaltyReceiver: string, royaltyFeeNumerator: string, implicitModeValidator: string, implicitModeProjectId: string,
...
const erc721ItemsInitializeSchema = {
  ...
  body: {
    type: 'object',
    required: ['owner', 'tokenName', 'tokenSymbol', 'tokenBaseURI', 'tokenContractURI', 'royaltyReceiver', 'royaltyFeeNumerator', 'implicitModeValidator', 'implicitModeProjectId'],
    properties: {
      owner: { type: 'string' },
      tokenName: { type: 'string' },
      tokenSymbol: { type: 'string' },
      tokenBaseURI: { type: 'string' },
      tokenContractURI: { type: 'string' },
      royaltyReceiver: { type: 'string' },
      royaltyFeeNumerator: { type: 'string' },
      implicitModeValidator: { type: 'string' },
      implicitModeProjectId: { type: 'string' },
...
export async function erc721ItemsInitialize(fastify: FastifyInstance) {
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      try {
        const { owner, tokenName, tokenSymbol, tokenBaseURI, tokenContractURI, royaltyReceiver, royaltyFeeNumerator, implicitModeValidator, implicitModeProjectId, waitForReceipt } = request.body
        ...
        const encodedData = contract.interface.encodeFunctionData('initialize', [
          owner, tokenName, tokenSymbol, tokenBaseURI, tokenContractURI, royaltyReceiver, BigInt(royaltyFeeNumerator), implicitModeValidator, implicitModeProjectId
        ])
        ...
        await txService.createTransaction({
          ...
          txHash: txHash,
          isDeployTx: false,
          args: [owner, tokenName, tokenSymbol, tokenBaseURI, tokenContractURI, royaltyReceiver, String(royaltyFeeNumerator), implicitModeValidator, implicitModeProjectId],
