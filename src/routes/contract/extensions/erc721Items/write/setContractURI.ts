...
type erc721ItemsSetContractURIRequestBody = {
  tokenContractURI: string,
  waitForReceipt?: boolean
}

type erc721ItemsSetContractURIRequestParams = {
  chainId: string
  contractAddress: string
}

type erc721ItemsSetContractURIResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc721ItemsSetContractURISchema = {
  tags: ['erc721Items'],
  body: {
    type: 'object',
    required: ['tokenContractURI'],
    properties: {
...
export async function erc721ItemsSetContractURI(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc721ItemsSetContractURIRequestParams
    Body: erc721ItemsSetContractURIRequestBody
    Reply: erc721ItemsSetContractURIResponse
  }>(
    '/write/erc721Items/:chainId/:contractAddress/setContractURI',
    { schema: erc721ItemsSetContractURISchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        const { tokenContractURI, waitForReceipt } = request.body
        ...
        const encodedData = contract.interface.encodeFunctionData('setContractURI', [
          tokenContractURI
        ])
        ...
        logStep(request, 'Sending setContractURI transaction...')
        const txResponse: TransactionResponse = await signer.sendTransaction(
          tx,
          { waitForReceipt: waitForReceipt ?? false }
        )
        txHash = txResponse.hash
        logStep(request, 'SetContractURI transaction sent', { txHash: txResponse.hash })
