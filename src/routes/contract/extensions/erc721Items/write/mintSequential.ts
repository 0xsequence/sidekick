  ...
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const encodedData = contract.interface.encodeFunctionData('mintSequential', [
          to, BigInt(amount)
        ])
        ...
        logStep(request, 'MintSequential transaction sent', { txHash: txResponse.hash })
