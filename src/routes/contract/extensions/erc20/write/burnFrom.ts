...
type erc20BurnFromRequestBody = {
  account: string, value: string,
  waitForReceipt?: boolean
}

type erc20BurnFromRequestParams = {
  chainId: string
  contractAddress: string
}

type erc20BurnFromResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
export async function erc20BurnFrom(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc20BurnFromRequestParams
    Body: erc20BurnFromRequestBody
    Reply: erc20BurnFromResponse
  }>(
    '/write/erc20/:chainId/:contractAddress/burnFrom',
    { schema: erc20BurnFromSchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        const { account, value, waitForReceipt } = request.body
        ...
        const encodedData = contract.interface.encodeFunctionData('burnFrom', [
        ...
        logStep(request, 'Sending burnFrom transaction...')
        const txResponse: TransactionResponse = await signer.sendTransaction(
          tx,
          { waitForReceipt: waitForReceipt ?? false }
        )
        txHash = txResponse.hash
        logStep(request, 'BurnFrom transaction sent', { txHash: txResponse.hash })
