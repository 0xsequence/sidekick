...
type erc20BurnRequestBody = {
  value: string,
  waitForReceipt?: boolean
}

type erc20BurnRequestParams = {
  chainId: string
  contractAddress: string
}

type erc20BurnResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
export async function erc20Burn(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc20BurnRequestParams
    Body: erc20BurnRequestBody
    Reply: erc20BurnResponse
  }>(
    '/write/erc20/:chainId/:contractAddress/burn',
    { schema: erc20BurnSchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        const { value, waitForReceipt } = request.body
        ...
        logStep(request, 'Burn transaction sent', { txHash: txResponse.hash })
