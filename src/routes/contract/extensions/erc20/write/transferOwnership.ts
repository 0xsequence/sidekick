...
type erc20TransferOwnershipRequestBody = {
  newOwner: string,
  waitForReceipt?: boolean
}

type erc20TransferOwnershipRequestParams = {
  chainId: string
  contractAddress: string
}

type erc20TransferOwnershipResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc20TransferOwnershipSchema = {
  tags: ['erc20'],
  body: {
    type: 'object',
    required: ['newOwner'],
...
export async function erc20TransferOwnership(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc20TransferOwnershipRequestParams
    Body: erc20TransferOwnershipRequestBody
    Reply: erc20TransferOwnershipResponse
  }>(
    '/write/erc20/:chainId/:contractAddress/transferOwnership',
    { schema: erc20TransferOwnershipSchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        const { newOwner, waitForReceipt } = request.body
        ...
        const encodedData = contract.interface.encodeFunctionData('transferOwnership', [
          newOwner
        ])
        ...
        logStep(request, 'Sending transferOwnership transaction...')
        const txResponse: TransactionResponse = await signer.sendTransaction(
          tx,
          { waitForReceipt: waitForReceipt ?? false }
        )
        txHash = txResponse.hash
        logStep(request, 'TransferOwnership transaction sent', { txHash: txResponse.hash })
