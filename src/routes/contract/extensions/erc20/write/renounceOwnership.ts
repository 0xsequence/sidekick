...
type erc20RenounceOwnershipRequestBody = {
  
  waitForReceipt?: boolean
}

type erc20RenounceOwnershipRequestParams = {
  chainId: string
  contractAddress: string
}

type erc20RenounceOwnershipResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc20RenounceOwnershipSchema = {
  tags: ['erc20'],
  body: {
    type: 'object',
    required: [],
...
export async function erc20RenounceOwnership(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc20RenounceOwnershipRequestParams
    Body: erc20RenounceOwnershipRequestBody
    Reply: erc20RenounceOwnershipResponse
  }>(
    '/write/erc20/:chainId/:contractAddress/renounceOwnership',
    { schema: erc20RenounceOwnershipSchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        const { waitForReceipt } = request.body
        ...
        const encodedData = contract.interface.encodeFunctionData('renounceOwnership', [
        ...
        logStep(request, 'Sending renounceOwnership transaction...')
        const txResponse: TransactionResponse = await signer.sendTransaction(
          tx,
          { waitForReceipt: waitForReceipt ?? false }
        )
        txHash = txResponse.hash
        logStep(request, 'RenounceOwnership transaction sent', { txHash: txResponse.hash })
