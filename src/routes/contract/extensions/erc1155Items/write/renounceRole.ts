...
type erc1155ItemsRenounceRoleRequestBody = {
  role: string, account: string,
  waitForReceipt?: boolean
}

type erc1155ItemsRenounceRoleRequestParams = {
  chainId: string
  contractAddress: string
}

type erc1155ItemsRenounceRoleResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc1155ItemsRenounceRoleSchema = {
  tags: ['erc1155Items'],
  body: {
    type: 'object',
...
export async function erc1155ItemsRenounceRole(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc1155ItemsRenounceRoleRequestParams
    Body: erc1155ItemsRenounceRoleRequestBody
    Reply: erc1155ItemsRenounceRoleResponse
  }>(
    '/write/erc1155Items/:chainId/:contractAddress/renounceRole',
    { schema: erc1155ItemsRenounceRoleSchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        ...
        const encodedData = contract.interface.encodeFunctionData('renounceRole', [
          role, account
        ])
        ...
        logStep(request, 'Sending renounceRole transaction...')
        const txResponse: TransactionResponse = await signer.sendTransaction(
          tx,
          { waitForReceipt: waitForReceipt ?? false }
        )
        txHash = txResponse.hash
        logStep(request, 'RenounceRole transaction sent', { txHash: txResponse.hash })
