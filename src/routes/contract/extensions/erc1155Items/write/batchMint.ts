...
type erc1155ItemsBatchMintRequestBody = {
  to: string, tokenIds: string[], amounts: string[], data: string,
  waitForReceipt?: boolean
}

type erc1155ItemsBatchMintRequestParams = {
  chainId: string
  contractAddress: string
}

type erc1155ItemsBatchMintResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc1155ItemsBatchMintSchema = {
  tags: ['erc1155Items'],
  body: {
    type: 'object',
    required: ['to', 'tokenIds', 'amounts', 'data'],
    properties: {
      to: { type: 'string' },
...
export async function erc1155ItemsBatchMint(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc1155ItemsBatchMintRequestParams
    Body: erc1155ItemsBatchMintRequestBody
    Reply: erc1155ItemsBatchMintResponse
  }>(
    '/write/erc1155Items/:chainId/:contractAddress/batchMint',
    { schema: erc1155ItemsBatchMintSchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        const { to, tokenIds, amounts, data, waitForReceipt } = request.body
        ...
        const encodedData = contract.interface.encodeFunctionData('batchMint', [
          to, tokenIds.map((v) => BigInt(v)), amounts.map((v) => BigInt(v)), data
        ])
        ...
        logStep(request, 'Sending batchMint transaction...')
        const txResponse: TransactionResponse = await signer.sendTransaction(
          tx,
          { waitForReceipt: waitForReceipt ?? false }
        )
        txHash = txResponse.hash
        logStep(request, 'BatchMint transaction sent', { txHash: txResponse.hash })
        ...
        await txService.createTransaction({
          ...
          txHash: txHash,
          isDeployTx: false,
          args: [to, tokenIds.map(String), amounts.map(String), data],
