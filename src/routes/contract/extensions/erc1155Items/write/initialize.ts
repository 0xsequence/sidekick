...
type erc1155ItemsInitializeRequestBody = {
  owner: string, tokenName: string, tokenBaseURI: string, tokenContractURI: string, royaltyReceiver: string, royaltyFeeNumerator: string, implicitModeValidator: string, implicitModeProjectId: string,
...
const erc1155ItemsInitializeSchema = {
  ...
  body: {
    type: 'object',
    required: ['owner', 'tokenName', 'tokenBaseURI', 'tokenContractURI', 'royaltyReceiver', 'royaltyFeeNumerator', 'implicitModeValidator', 'implicitModeProjectId'],
    properties: {
      owner: { type: 'string' },
...
export async function erc1155ItemsInitialize(fastify: FastifyInstance) {
  fastify.post<{
    ...
    async (request, reply) => {
      ...
      try {
        const { owner, tokenName, tokenBaseURI, tokenContractURI, royaltyReceiver, royaltyFeeNumerator, implicitModeValidator, implicitModeProjectId, waitForReceipt } = request.body
        ...
        const encodedData = contract.interface.encodeFunctionData('initialize', [
          owner, tokenName, tokenBaseURI, tokenContractURI, royaltyReceiver, BigInt(royaltyFeeNumerator), implicitModeValidator, implicitModeProjectId
        ])
        ...
        await txService.createTransaction({
          ...
          txHash: txHash,
          isDeployTx: false,
          args: [owner, tokenName, tokenBaseURI, tokenContractURI, royaltyReceiver, String(royaltyFeeNumerator), implicitModeValidator, implicitModeProjectId],
