...
type erc1155ItemsSetContractNameRequestBody = {
  tokenName: string,
  waitForReceipt?: boolean
}

type erc1155ItemsSetContractNameRequestParams = {
  chainId: string
  contractAddress: string
}

type erc1155ItemsSetContractNameResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc1155ItemsSetContractNameSchema = {
  tags: ['erc1155Items'],
  body: {
    type: 'object',
    required: ['tokenName'],
...
export async function erc1155ItemsSetContractName(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc1155ItemsSetContractNameRequestParams
    Body: erc1155ItemsSetContractNameRequestBody
    Reply: erc1155ItemsSetContractNameResponse
  }>(
    '/write/erc1155Items/:chainId/:contractAddress/setContractName',
    { schema: erc1155ItemsSetContractNameSchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        const { tokenName, waitForReceipt } = request.body
        ...
        const encodedData = contract.interface.encodeFunctionData('setContractName', [
          tokenName
        ])
        ...
        logStep(request, 'Sending setContractName transaction...')
        const txResponse: TransactionResponse = await signer.sendTransaction(
          tx,
          { waitForReceipt: waitForReceipt ?? false }
        )
        txHash = txResponse.hash
        logStep(request, 'SetContractName transaction sent', { txHash: txResponse.hash })
