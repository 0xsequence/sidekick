...
type erc1155ItemsSetBaseMetadataURIRequestBody = {
  tokenBaseURI: string,
  waitForReceipt?: boolean
}

type erc1155ItemsSetBaseMetadataURIRequestParams = {
  chainId: string
  contractAddress: string
}

type erc1155ItemsSetBaseMetadataURIResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc1155ItemsSetBaseMetadataURISchema = {
  tags: ['erc1155Items'],
  body: {
    type: 'object',
    required: ['tokenBaseURI'],
    properties: {
...
export async function erc1155ItemsSetBaseMetadataURI(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc1155ItemsSetBaseMetadataURIRequestParams
    Body: erc1155ItemsSetBaseMetadataURIRequestBody
    Reply: erc1155ItemsSetBaseMetadataURIResponse
  }>(
    '/write/erc1155Items/:chainId/:contractAddress/setBaseMetadataURI',
    { schema: erc1155ItemsSetBaseMetadataURISchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        const { tokenBaseURI, waitForReceipt } = request.body
        ...
        const encodedData = contract.interface.encodeFunctionData('setBaseMetadataURI', [
          tokenBaseURI
        ])
        ...
        logStep(request, 'Sending setBaseMetadataURI transaction...')
        const txResponse: TransactionResponse = await signer.sendTransaction(
          tx,
          { waitForReceipt: waitForReceipt ?? false }
        )
        txHash = txResponse.hash
        logStep(request, 'SetBaseMetadataURI transaction sent', { txHash: txResponse.hash })
