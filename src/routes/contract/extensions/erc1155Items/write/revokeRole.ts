...
type erc1155ItemsRevokeRoleRequestBody = {
  role: string, account: string,
  waitForReceipt?: boolean
}

type erc1155ItemsRevokeRoleRequestParams = {
  chainId: string
  contractAddress: string
}

type erc1155ItemsRevokeRoleResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc1155ItemsRevokeRoleSchema = {
  tags: ['erc1155Items'],
  body: {
    type: 'object',
...
export async function erc1155ItemsRevokeRole(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc1155ItemsRevokeRoleRequestParams
    Body: erc1155ItemsRevokeRoleRequestBody
    Reply: erc1155ItemsRevokeRoleResponse
  }>(
    '/write/erc1155Items/:chainId/:contractAddress/revokeRole',
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const encodedData = contract.interface.encodeFunctionData('revokeRole', [
          role, account
        ])
        ...
        logStep(request, 'Sending revokeRole transaction...')
        const txResponse: TransactionResponse = await signer.sendTransaction(
          tx,
          { waitForReceipt: waitForReceipt ?? false }
        )
        txHash = txResponse.hash
        logStep(request, 'RevokeRole transaction sent', { txHash: txResponse.hash })
