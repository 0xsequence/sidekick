...
type erc1155ItemsSafeBatchTransferFromRequestBody = {
  from: string, to: string, ids: string[], amounts: string[], data: string,
  waitForReceipt?: boolean
}

type erc1155ItemsSafeBatchTransferFromRequestParams = {
  chainId: string
  contractAddress: string
}

type erc1155ItemsSafeBatchTransferFromResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc1155ItemsSafeBatchTransferFromSchema = {
  tags: ['erc1155Items'],
  body: {
    type: 'object',
    required: ['from', 'to', 'ids', 'amounts', 'data'],
    properties: {
      from: { type: 'string' },
      to: { type: 'string' },
...
export async function erc1155ItemsSafeBatchTransferFrom(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc1155ItemsSafeBatchTransferFromRequestParams
    Body: erc1155ItemsSafeBatchTransferFromRequestBody
    Reply: erc1155ItemsSafeBatchTransferFromResponse
  }>(
    '/write/erc1155Items/:chainId/:contractAddress/safeBatchTransferFrom',
    { schema: erc1155ItemsSafeBatchTransferFromSchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        const { from, to, ids, amounts, data, waitForReceipt } = request.body
        ...
        const encodedData = contract.interface.encodeFunctionData('safeBatchTransferFrom', [
          from, to, ids.map((v) => BigInt(v)), amounts.map((v) => BigInt(v)), data
        ])
        ...
        logStep(request, 'Sending safeBatchTransferFrom transaction...')
        const txResponse: TransactionResponse = await signer.sendTransaction(
          tx,
          { waitForReceipt: waitForReceipt ?? false }
        )
        txHash = txResponse.hash
        logStep(request, 'SafeBatchTransferFrom transaction sent', { txHash: txResponse.hash })
        ...
        await txService.createTransaction({
          ...
          txHash: txHash,
          isDeployTx: false,
          args: [from, to, ids.map(String), amounts.map(String), data],
