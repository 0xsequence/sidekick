...
type erc721SetApprovalForAllRequestBody = {
  operator: string, _approved: string,
  waitForReceipt?: boolean
}

type erc721SetApprovalForAllRequestParams = {
  chainId: string
  contractAddress: string
}

type erc721SetApprovalForAllResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc721SetApprovalForAllSchema = {
  tags: ['erc721'],
  body: {
    type: 'object',
    required: ['operator', '_approved'],
    properties: {
      operator: { type: 'string' },
      _approved: { type: 'string' },
...
export async function erc721SetApprovalForAll(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc721SetApprovalForAllRequestParams
    Body: erc721SetApprovalForAllRequestBody
    Reply: erc721SetApprovalForAllResponse
  }>(
    '/write/erc721/:chainId/:contractAddress/setApprovalForAll',
    ...
    async (request, reply) => {
      ...
      try {
        const { operator, _approved, waitForReceipt } = request.body
