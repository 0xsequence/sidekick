...
type erc721ItemsSetImplicitModeValidatorRequestBody = {
  validator: string,
  waitForReceipt?: boolean
}

type erc721ItemsSetImplicitModeValidatorRequestParams = {
  chainId: string
  contractAddress: string
}

type erc721ItemsSetImplicitModeValidatorResponse = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc721ItemsSetImplicitModeValidatorSchema = {
  tags: ['ERC721Items'],
  body: {
    type: 'object',
    required: ['validator'],
...
export async function erc721ItemsSetImplicitModeValidator(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc721ItemsSetImplicitModeValidatorRequestParams
    Body: erc721ItemsSetImplicitModeValidatorRequestBody
    Reply: erc721ItemsSetImplicitModeValidatorResponse
  }>(
    '/write/erc721Items/:chainId/:contractAddress/setImplicitModeValidator',
    { schema: erc721ItemsSetImplicitModeValidatorSchema },
    async (request, reply) => {
      logRequest(request)
      ...
      try {
        const { validator, waitForReceipt } = request.body
        ...
        const encodedData = contract.interface.encodeFunctionData('setImplicitModeValidator', [
          validator
        ])
        ...
        logStep(request, 'Sending setImplicitModeValidator transaction...')
        const txResponse: TransactionResponse = await signer.sendTransaction(
          tx,
          { waitForReceipt: waitForReceipt ?? false }
        )
        txHash = txResponse.hash
        logStep(request, 'SetImplicitModeValidator transaction sent', { txHash: txResponse.hash })
        ...
        await txService.createTransaction({
          ...
          isDeployTx: false,
          args: [validator],
          functionName: 'setImplicitModeValidator'
        })
