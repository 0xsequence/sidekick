...
type erc721ItemsSafeTransferFrom_2RequestBody = {
  from: string, to: string, id: string, data: string,
  waitForReceipt?: boolean
}

type erc721ItemsSafeTransferFrom_2RequestParams = {
  chainId: string
  contractAddress: string
}

type erc721ItemsSafeTransferFrom_2Response = {
  result?: {
    txHash: string | null
    txUrl: string | null
...
const erc721ItemsSafeTransferFrom_2Schema = {
  tags: ['ERC721Items'],
  body: {
    type: 'object',
    required: ['from', 'to', 'id', 'data'],
    properties: {
      from: { type: 'string' },
      to: { type: 'string' },
      id: { type: 'string' },
...
export async function erc721ItemsSafeTransferFrom_2(fastify: FastifyInstance) {
  fastify.post<{
    Params: erc721ItemsSafeTransferFrom_2RequestParams
    Body: erc721ItemsSafeTransferFrom_2RequestBody
    Reply: erc721ItemsSafeTransferFrom_2Response
  }>(
    '/write/erc721Items/:chainId/:contractAddress/safeTransferFrom_2',
    ...
    async (request, reply) => {
      ...
      try {
        const { from, to, id, data, waitForReceipt } = request.body
